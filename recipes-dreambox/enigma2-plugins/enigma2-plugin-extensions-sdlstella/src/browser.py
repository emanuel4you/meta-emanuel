...
config.plugins.stella.romlocation = ConfigDirectory(default="/media/hdd")

#-----------------------------------------------------------------------------------

class StellaBrowserSummary(Screen):
	def __init__(self, session, parent):
		Screen.__init__(self, session, parent = parent)
		self.skinName = ["StellaBrowserSummary"]

#-----------------------------------------------------------------------------------

class StellaBrowser(Screen):
	def __init__(self, session, filemode=True):
		Screen.__init__(self, session)
		self.skinName = ["StellaBrowser"]
		self.__filemode = filemode
		self["title"] = StaticText("Stella Browser")
		self["lcdinfo"] = StaticText()
		if self.__filemode:
			self.filelist = FileList(config.plugins.stella.romlocation.value, matchingPattern = "^.*\.(bin|BIN)", showDirectories = True)
		else:
			self.filelist = FileList(config.plugins.stella.romlocation.value, showDirectories = True, showFiles = False)
	...
	def __setTitle(self):
		if self.__filemode:
			self.setTitle(_("Select a bin rom for Stella"))
		else:
	...
	def KeyGreen(self):
		if self.__filemode:
			...
			config.plugins.stella.romlocation.value = self["filelist"].getCurrentDirectory()
