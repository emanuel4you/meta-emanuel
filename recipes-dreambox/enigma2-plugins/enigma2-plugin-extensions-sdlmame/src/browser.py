...
config.plugins.mame.romlocation = ConfigDirectory(default="/root")

#-----------------------------------------------------------------------------------

class MameBrowserSummary(Screen):
	def __init__(self, session, parent):
		Screen.__init__(self, session, parent = parent)
		self.skinName = ["MameBrowserSummary"]

#-----------------------------------------------------------------------------------

class MameBrowser(Screen):
	def __init__(self, session, filemode=True):
		Screen.__init__(self, session)
		self.skinName = ["MameBrowser"]
		self.__filemode = filemode
		self["title"] = StaticText("Mame Rom Browser")
		self["lcdinfo"] = StaticText()
		if self.__filemode:
			self.filelist = FileList(config.plugins.mame.romlocation.value, matchingPattern = "^.*\.(zip|ZIP)", showDirectories = True)
		else:
			self.filelist = FileList(config.plugins.mame.romlocation.value, showDirectories = True, showFiles = False)
	...
	def __setTitle(self):
		if self.__filemode:
			self.setTitle(_("Select a rom for MAME"))
		else:
	...
	def KeyGreen(self):
		if self.__filemode:
			...
			config.plugins.mame.romlocation.value = self["filelist"].getCurrentDirectory()
